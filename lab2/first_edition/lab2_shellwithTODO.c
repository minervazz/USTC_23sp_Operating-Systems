...
int exec_builtin(int argc, char**argv, int *fd) {
    ...
    if (strcmp(argv[0], "cd") == 0) {
        if (chdir(argv[1] != 0)) {
            printf("cd: No such file or directory: %s", argv[1]);  // i wrote this line and the one below
            return -1;
        }
    } else if (strcmp(argv[0], "exit") == 0){
       exit(0); // i wrote this line
    } else {
        // 不是内置指令时
        return -1;
    }
...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            //TODO : 打开输出文件从头写入
            tfd = open(argv[i + 1], O_RDWR | O_CREAT | O_TRUNC, 0666); // i wrote this line
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } else {
                //TODO : 输出重定向
                fd[WRITE_END] = tfd; // i wrote this line
            }
            i += 2;
        } else if(strcmp(argv[i], ">>") == 0) {
            //TODO : 打开输出文件追加写入
            tfd = open(argv[i + 1], O_RDWR | O_CREAT | O_APPEND, 0666); // i wrote this
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } else {
                //TODO :输出重定向
                fd[WRITE_END] = tfd; // i wrote this
            }
            i += 2;
        } else if(strcmp(argv[i], "<") == 0) {
            //TODO : 读输入文件
            tfd = open(argv[i + 1], O_RDONLY); // i
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } else {
                //TODO :输出重定向
...
int execute(int argc, char** argv) {
    ...
    /* TODO :运行命令与结束 */
    execvp(argv[0], argv); // i and below
    return 0;
}
