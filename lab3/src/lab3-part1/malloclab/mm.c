...
double get_utilization() { // 计算内存利用率
...
int mm_init(void)
{
    ...
    heap_listp += (2 * WSIZE);
    // 扩展堆空间到 CHUNKSIZE / WSIZE 大小
    if (extend_heap(CHUNKSIZE / WSIZE) == NULL)
        return -1;
    /* mm_check(__FUNCTION__);*/
    heap_size += 4 * WSIZE + CHUNKSIZE; // 更新堆的总大小
    return 0;
}
...
void *mm_malloc(size_t size)
{
    ...
    if ((bp = find_fit_first(newsize)) != NULL) // 需要改动成best fit
    {
        place(bp, newsize);
        return bp;
    }
    ...
    heap_size += extend_size; // renew 扩展了的 heap_size
    place(bp, newsize);
...
static void *coalesce(void *bp)
{
    ...
    */
    ...
    {
        add_to_free_list(bp); // 将当前块加入空闲链表
    }
    else if (prev_alloc && !next_alloc) /* 前块已分配，后块空闲 */
    {
        delete_from_free_list(NEXT_BLKP(bp));  // 从空闲链表中删除下一个块
        PUT(HDRP(bp), PACK(size + GET_SIZE(HDRP(NEXT_BLKP(bp))), 1, 0));  // 更新当前块头部大小和状态
        PUT(FTRP(bp), PACK(GET_SIZE(HDRP(bp)), 1, 0));  // 更新当前块尾部大小和状态
        add_to_free_list(bp);  // 将当前块加入空闲链表
    }
    else if (!prev_alloc && next_alloc) /* 前块空闲，后块已分配 */
    {
        delete_from_free_list(PREV_BLKP(bp));  // 从空闲链表中删除前一个块
        bp = PREV_BLKP(bp);  // 更新当前块指针为前一个块
        PUT(HDRP(bp), PACK(GET_SIZE(HDRP(bp)) + GET_SIZE(HDRP(NEXT_BLKP(bp))), 1, 0));  // 更新当前块头部大小和状态
        PUT(FTRP(bp), PACK(GET_SIZE(HDRP(bp)), 1, 0));  // 更新当前块尾部大小和状态
        add_to_free_list(bp);  // 将当前块加入空闲链表
    }
    else /* 前后都是空闲块 */
    {
        delete_from_free_list(NEXT_BLKP(bp));  // 从空闲链表中删除下一个块
        PUT(HDRP(bp), PACK(size + GET_SIZE(HDRP(NEXT_BLKP(bp))), 0, 0));  // 更新当前块头部大小和状态
        PUT(FTRP(bp), PACK(GET_SIZE(HDRP(bp)), 0, 0));  // 更新当前块尾部大小和状态
        delete_from_free_list(PREV_BLKP(bp));  // 从空闲链表中删除前一个块
        bp = PREV_BLKP(bp);  // 更新当前块指针为前一个块
        PUT(HDRP(bp), PACK(GET_SIZE(HDRP(bp)) + GET_SIZE(HDRP(NEXT_BLKP(bp))), 1, 0));  // 更新当前块头部大小和状态
        PUT(FTRP(bp), PACK(GET_SIZE(HDRP(bp)), 1, 0));  // 更新当前块尾部大小和状态
...
static void *find_fit_first(size_t asize) 
{
    ...
    */
    char *p = free_listp; // 从空闲链表的起始位置开始遍历
    while (p != NULL) {
        if (GET_SIZE(HDRP(p)) >= asize) { // 如果当前空闲块的大小大于等于需要分配的块大小
            return p; // 返回该空闲块的地址
        }
        p = (char *)GET_SUCC(p); // 获取下一个空闲块的地址
    }
    return NULL; // 如果没有找到合适的空闲块，则返回空指针
}
...
static void* find_fit_best(size_t asize) {
    ...
    */
    char *p = free_listp; // 从空闲链表的起始位置开始遍历
    char *best = NULL; // 最佳适配的空闲块指针
    while (p != NULL) {
        if (GET_SIZE(HDRP(p)) >= asize) { // 如果当前空闲块的大小大于等于需要分配的块大小
            if (best == NULL || GET_SIZE(HDRP(p)) < GET_SIZE(HDRP(best))) { 
                // 如果best为空或者当前空闲块的大小更小，更新best为当前空闲块
                best = p;
            }
        }
        p = (char*)GET_SUCC(p); // 获取下一个空闲块的地址
    }
    return best; // 返回最佳适配的空闲块的地址，如果没有找到合适的空闲块则返回NULL
}
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    delete_from_free_list(bp); // 从空闲链表中删除该块

    size_t size = GET_SIZE(HDRP(bp)); // 获取当前空闲块的大小

    if (size - asize < MIN_BLK_SIZE) {
        // 如果剩余空间不足以容纳一个最小的空闲块大小，将整个空闲块都分配出去
        PUT(HDRP(bp), PACK(size, 1, 1)); // 更新头部块信息，设置为已分配块
        user_malloc_size += size - WSIZE; // 更新用户分配的总大小
    } else {
        // 如果剩余空间可以作为一个新的空闲块
        PUT(HDRP(bp), PACK(asize, 1, 1)); // 更新头部块信息，设置为已分配块
        PUT(HDRP(NEXT_BLKP(bp)), PACK(size - asize, 1, 0)); // 更新剩余空闲块的头部信息，设置为未分配块
        PUT(FTRP(NEXT_BLKP(bp)), PACK(size - asize, 1, 0)); // 更新剩余空闲块的尾部信息，设置为未分配块
        add_to_free_list(NEXT_BLKP(bp)); // 将剩余空闲块添加到空闲链表中
        user_malloc_size += asize - WSIZE; // 更新用户分配的总大小
    }
    // 设置下一块的前驱已分配位
