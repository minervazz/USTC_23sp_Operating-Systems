...
cluster_t read_fat_entry(cluster_t clus)
{
    ...
    // TODO :1. 计算簇号 clus 对应的 FAT 表项的偏移量，并计算表项所在的扇区号
    uint32_t fat_offset = clus * 4; // 在 FAT32 中，每个表项占 4 字节
    uint32_t fat_sector = FIRST_FAT_SECTOR + (fat_offset / PHYSICAL_SECTOR_SIZE);
    uint32_t entry_offset = fat_offset % PHYSICAL_SECTOR_SIZE;

    // 2. 使用 sector_read 函数读取该扇区
    sector_read(fat_sector, sector_buffer);

    // 3. 计算簇号 clus 对应的 FAT 表项在该扇区中的偏移量
    // 在此，我们已经在第一步中计算了 entry_offset。

    // 4. 从该偏移量处读取对应表项的值，并返回
    return *(cluster_t*)&sector_buffer[entry_offset];
}
...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for(size_t i = 0; i < sectors_count; i++) {
        // TODO :1. 使用 sector_read 函数读取从扇区号 from_sector 开始的第 i 个扇区
        ...
        for(size_t off = 0; off < PHYSICAL_SECTOR_SIZE; off += DIR_ENTRY_SIZE) {
            ...
            if (memcmp(dir->DIR_Name, name, len) == 0) {
                slot->sector = from_sector + i;
                slot->offset = off;
                return FIND_EXIST;
            }
            // 4. 如果不是待查找的目录项，检查该目录项是否为空，如果为空，将该目录项的信息填入 slot 中，并返回 FIND_EMPTY
            if (!empty_entry_found && dir->DIR_Name[0] == 0xE5 || dir->DIR_Name[0] == 0x00) {
                empty_entry_found = 1;
                slot->sector = from_sector + i;
                slot->offset = off;
            }
...
int find_entry_internal(const char* path, DirEntrySlot* slot, const char** remains) {
    ...
    *remains += strspn(*remains, "/");    // 跳过开头的'/'
    ...
    while (**remains != '\0' && state == FIND_EXIST) {
        ...
        if(level == 0) {
            // TODO :1.1: 设置根目录的扇区号和扇区数
            sector_t root_sec = meta.root_dir_sectors;   // 获取根目录的扇区号
            size_t nsec = meta.root_dir_sectors_count;   // 获取根目录的扇区数
            ...
            while (is_cluster_inuse(clus)) {
                ...
                sector_t clus_sector = cluster_to_sector(clus);   // 转换簇号为扇区号
                state = find_entry_in_sectors(*remains, len, clus_sector, meta.sectors_per_cluster, slot);
                if(state < 0) { // 出现错误
                    return state;
    ...
    while (root || is_cluster_inuse(clus)) {
        ...
        for(size_t i=0; i < nsec; i++) {
            ...
            for(size_t off=0; off < meta.sector_size; off += DIR_ENTRY_SIZE) {
                DIR_ENTRY* dir_entry = (DIR_ENTRY*)(sector_buffer + off);
                // 1. 确认其是否是表示文件或目录的项
                if(is_valid_dir_entry(dir_entry)) {
                    // 2. 从 FAT 文件名中，获得长文件名
                    to_longname(dir_entry->DIR_Name, name);
                    // 3. 使用 filler 填入 buf
                    if(filler(buf, name, NULL, 0) != 0) {
                        return -ENOMEM;
                    }
                } else if(dir_entry->DIR_Name[0] == '\0') {
                    // 4. 找到空项即可结束查找
                    return 0;
                }
            }
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
    ...
    while(p < size) {
        off_t clus_offset = offset % (meta.sec_per_clus * meta.sector_size);
        size_t clus_size = min(meta.sec_per_clus * meta.sector_size - clus_offset, size - p);
        size_t read_size = read_from_cluster_at_offset(clus, clus_offset, clus_size, buffer + p);
        p += read_size;
        offset += read_size;

        if(read_size < clus_size) {
            break;
        }
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    for(size_t i = 0; i < meta.fats; i++) {
        ...
        sector_t sec = meta.reserved_sectors + i * meta.fat_size_sectors + clus_sec;
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    for(cluster_t i = 2; i < meta.data_clusters + 2; i++) {
        cluster_t fat_entry;
        read_fat_entry(i, &fat_entry);
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    DIR_ENTRY dot;
    dir_entry_create(&dot, DOT_NAME, ATTR_DIRECTORY, clus, 2 * sizeof(DIR_ENTRY));
    cluster_write(clus, 0, (char*)&dot, sizeof(DIR_ENTRY));

    DIR_ENTRY dotdot;
    dir_entry_create(&dotdot, DOTDOT_NAME, ATTR_DIRECTORY, 0, 2 * sizeof(DIR_ENTRY));
    cluster_write(clus, sizeof(DIR_ENTRY), (char*)&dotdot, sizeof(DIR_ENTRY));

    ret = fat16_mknod(path, mode, clus, ATTR_DIRECTORY, 2 * sizeof(DIR_ENTRY));
    if(ret < 0) {
        return ret;
    }
...
int fat16_rmdir(const char *path) {
    ...
    DIR_ENTRY buf[16]; // suppose the cluster size is 512
    size_t read_bytes = cluster_read(dir->DIR_FstClusLO, 0, (char*)buf, sizeof(buf));
    if (read_bytes < 2 * sizeof(DIR_ENTRY)) {
        return -EIO; // Error: cannot read the directory entries
    }

    // Check if directory is empty (exclude "." and "..")
    for (int i = 2; i < read_bytes / sizeof(DIR_ENTRY); ++i) {
        if (buf[i].DIR_Name[0] != SLOT_EMPTY && buf[i].DIR_Name[0] != SLOT_DELETED) {
            return -ENOTEMPTY;
        }
    }

    // Clear the cluster
    ret = cluster_clear(dir->DIR_FstClusLO);
    if (ret < 0) {
        return ret;
    }

    // Write CLUSTER_FREE to FAT
    ret = write_fat_entry(dir->DIR_FstClusLO, CLUSTER_FREE);
    if (ret < 0) {
        return ret;
    }

    // Delete the directory entry in parent directory
    dir->DIR_Name[0] = SLOT_DELETED;
    ret = dir_entry_write(slot);
    if (ret < 0) {
        return ret;
    }
...
ssize_t write_to_cluster_at_offset(cluster_t clus, off_t offset, const char* data, size_t size) {
    ...
    size_t sec_idx = offset / meta.sector_size;  // 计算第一个扇区索引
    size_t sec_off = offset % meta.sector_size;  // 计算第一个扇区偏移量

    // 遍历所有涉及的扇区
    for (; pos < size;) {
        sector_t sec = cluster_to_sector(clus) + sec_idx;
        int ret = sector_read(sec, sector_buffer);  // 读取扇区内容
        if (ret < 0) {
            return ret;  // 读取失败，返回错误代码
        }

        // 计算此次要写入的数据大小
        size_t write_size = meta.sector_size - sec_off;
        if (write_size > size - pos) {
            write_size = size - pos;
        }

        // 将数据写入扇区缓冲区
        memcpy(sector_buffer + sec_off, data + pos, write_size);
        ret = sector_write(sec, sector_buffer);  // 写入扇区
        if (ret < 0) {
            return ret;  // 写入失败，返回错误代码
        }
...
int file_reserve_clusters(DIR_ENTRY* dir, size_t size) {
    ...
    // 计算需要多少簇
    size_t needed_clusters = (size + meta.cluster_size - 1) / meta.cluster_size;

    // 如果文件没有簇，直接分配足够的簇
    if (dir->DIR_FstClusLO == 0) {
        cluster_t first_clus;
        int ret = alloc_clusters(needed_clusters, &first_clus);
        if (ret < 0) {
            return ret;  // 分配失败，返回错误代码
        }
        dir->DIR_FstClusLO = first_clus;
    }
    // 如果文件已有簇，找到最后一个簇（哪个簇是当前该文件的最后一个簇？），并计算需要额外分配多少个簇
    else {
        cluster_t cur_clus = dir->DIR_FstClusLO;
        cluster_t next_clus;
        size_t actual_clusters = 0;  // 文件已有的簇数

        while ((read_fat_entry(cur_clus, &next_clus) == 0) && (next_clus != CLUSTER_END)) {
            cur_clus = next_clus;
            actual_clusters++;
        }

        // 分配额外的簇，并将分配好的簇连在最后一个簇后
        if (needed_clusters > actual_clusters) {
            cluster_t first_clus;
            int ret = alloc_clusters(needed_clusters - actual_clusters, &first_clus);
            if (ret < 0) {
                return ret;  // 分配失败，返回错误代码
            }
            write_fat_entry(cur_clus, first_clus);
        }
    }
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
    ...
    int ret = find_entry(path, &slot);
    if(ret < 0) {
        return ret;  // 文件未找到，返回错误代码
    }

    // 检查是否是目录
    if(is_directory(slot.dir.DIR_Attr)) {
        return -EISDIR;  // 是目录，返回错误代码
    }

    // 确保文件足够大以包含所有写入的数据
    size_t final_size = offset + size;
    if(final_size > slot.dir.DIR_FileSize) {
        ret = file_reserve_clusters(&slot.dir, final_size);
        if(ret < 0) {
            return ret;  // 簇分配失败，返回错误代码
        }
    }

    // 写入数据
    cluster_t clus = slot.dir.DIR_FstClusLO;
    size_t clus_size = meta.cluster_size;
    size_t written_bytes = 0;
    while(written_bytes < size) {
        size_t clus_offset = offset % clus_size;
        size_t write_size = min(size - written_bytes, clus_size - clus_offset);
        ret = write_to_cluster_at_offset(clus, clus_offset, data + written_bytes, write_size);
        if(ret < 0) {
            return ret;  // 写入失败，返回错误代码
        }
        ...
        // 若写入过程跨越簇，获取下一个簇
        if(offset / clus_size > clus) {
            read_fat_entry(clus, &clus);
        }
    }

    // 更新目录项
    update_dir_entry(&slot);
...
int fat16_truncate(const char *path, off_t size, struct fuse_file_info* fi) {
    ...
    // 获取当前文件大小
    size_t cur_size = slot.dir.DIR_FileSize;

    if(size < cur_size) {
        // 如果需要的大小小于当前大小，释放多余的簇
        size_t required_clusters = (size + meta.cluster_size - 1) / meta.cluster_size;
        size_t current_clusters = (cur_size + meta.cluster_size - 1) / meta.cluster_size;

        // 获取要删除的第一个簇
        cluster_t clus_to_remove = slot.dir.DIR_FstClusLO;
        for(size_t i = 0; i < required_clusters; ++i) {
            read_fat_entry(clus_to_remove, &clus_to_remove);
        }

        // 释放多余的簇
        cluster_t next_clus;
        while(clus_to_remove != CLUSTER_END) {
            read_fat_entry(clus_to_remove, &next_clus);
            write_fat_entry(clus_to_remove, CLUSTER_FREE);
            clus_to_remove = next_clus;
        }

        // 将最后一个簇设置为结束簇
        if(required_clusters > 0) {
            cluster_t last_clus = slot.dir.DIR_FstClusLO;
            for(size_t i = 1; i < required_clusters; ++i) {
                read_fat_entry(last_clus, &last_clus);
            }
            write_fat_entry(last_clus, CLUSTER_END);
        }

        // 更新文件大小
        slot.dir.DIR_FileSize = size;
    } else if(size > cur_size) {
        // 如果需要的大小大于当前大小，分配更多的簇，并将新的部分置零
        ret = file_reserve_clusters(&slot.dir, size);
        if(ret < 0) {
            return ret;  // 簇分配失败，返回错误代码
        }

        // 将新的部分置零
        size_t bytes_to_zero = size - cur_size;
        char* zero_buffer = calloc(bytes_to_zero, 1);
        ret = write_to_cluster_at_offset(slot.dir.DIR_FstClusLO, cur_size, zero_buffer, bytes_to_zero);
        free(zero_buffer);
        if(ret < 0) {
            return ret;  // 写入失败，返回错误代码
        }
